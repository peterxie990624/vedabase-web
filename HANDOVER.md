...
> **Manus 须知**：当用户让Manus读取这个文档的时候，特别要读取**零、Manus工作原则**，而且要**注意【零】里面的每一个字，**特别要注意**加粗的部分**。每次提交后，将提交号和相对应的时间[年月日时分]发给用户

### 原则一：开启新任务时确认工作原则
每次开启新的 Manus 对话任务时以及继续之前的对话任务时，读取本文档后，去读取`logs_human_to_manus`下的日志，在足够了解项目历史对话的情况下，与用户有更加默契的配合，要保证每次任务是在了解了历史对话的前提下进行的，如果某次用户发出消息时你没有确保你知道本项目到底有多少个日志文件时，说明你忘记读取历史对话了，这时候去读取`logs_human_to_manus`下的所有日志。


### 原则二：自动化日志记录与交接文档维护

#### 原则：
每次在用户发出内容、回复用户的对话后和完成对话时，记录当前时间，并在回复人类后在文件夹 `logs_human_to_manus` 下自动按照日期创建/更新其中的文件，将用户的话****原封不动****地加入（包括用户发的图片，用户上传的文件仅用名字替代），对 Manus 的话的部分，摘取**每一段必要的内容**，包括在**每段最后做了哪些改动，进行了哪些必要和关键尝试**，原则是**摘取的内容也要原封不动**。图片上传到`./docs/chat_logs/logs_human_to_manus/images` ，先保留之前的命名，并在日志中引用这些图片。
