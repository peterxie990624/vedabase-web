...
在文件夹 `logs_human_to_manus` 下自动按照这个时间创建/更新其中的文件，分别将用户的话****原封不动****地加入（包括用户发的图片，用户上传的文件仅用名字替代），还有对Manus 发给用户内容，Manus 发给用户的部分，摘取**每一段必要的内容**，包括在**每段最后做了哪些改动，进行了哪些必要和关键尝试**，原则是**摘取的内容也要原封不动**。**图片上传到**`./docs/chat_logs/logs_human_to_manus/images` ，先保留之前的命名，并在日志中引用这些图片。
