...
在文件夹 `logs_human_to_manus` 下自动按照这个时间创建/更新其中的文件，分别将用户的话****原封不动、完整****地加入（包括用户发的图片，用户上传的文件仅用名字替代），还有对Manus执行任务期间所做的每一步都要能够获取到，并从中摘取**必要的内容**，包括在**每段最后做了【哪些改动】，进行了哪些【必要和关键尝试】**，原则是**摘取的内容也要原封不动、完整**。
