// 这是新的 searchBG 函数实现，将替换原有的
// 支持中英文映射、多位置高亮、结果合并

const searchBG_v2 = (keyword: string, q: string, bgData: any, language: Language, buildWordPairPreview: any): SearchResult[] => {
  if (!bgData) return [];
  const found: SearchResult[] = [];
  const kwLower = keyword.toLowerCase();
  const kwNorm = normalizeSanskrit(keyword);
  const isEnKeyword = /[a-zA-Z]/.test(q.trim());
  const isZhKeyword = /[\u4e00-\u9fff]/.test(q.trim());
